...
class TestMessageHandlerInitialization:
    ...
    def test_handler_initialization(
        self, test_config: Config, mock_github_client: GitHubClient
    ) -> None:
        """测试处理器初始化。"""
...
class TestExtractTags:
    ...
    def test_extract_deduplicate_tags(
        self, test_config: Config, mock_github_client: GitHubClient
    ) -> None:
        """测试标签去重（保持顺序）。"""
        handler = MessageHandler(test_config, mock_github_client)
    ...
    def test_exclude_journal_label(
        self, test_config: Config, mock_github_client: GitHubClient
    ) -> None:
        """测试排除 journal 标签。"""
        handler = MessageHandler(test_config, mock_github_client)
...
class TestBuildIssueContent:
    ...
    def test_build_issue_title_format(
        self, mock_datetime, test_config: Config, mock_github_client: GitHubClient
    ) -> None:
        """测试 Issue 标题格式。"""
        mock_now = MagicMock()
    ...
    def test_build_issue_body_with_text_only(
        self, test_config: Config, mock_github_client: GitHubClient
    ) -> None:
        """测试只有文本的 Issue 正文。"""
        handler = MessageHandler(test_config, mock_github_client)
    ...
    def test_build_issue_body_with_images_only(
        self, test_config: Config, mock_github_client: GitHubClient
    ) -> None:
        """测试只有图片的 Issue 正文。"""
        handler = MessageHandler(test_config, mock_github_client)
    ...
    def test_build_issue_body_with_text_and_images(
        self, test_config: Config, mock_github_client: GitHubClient
    ) -> None:
        """测试带文本和图片的 Issue 正文。"""
        handler = MessageHandler(test_config, mock_github_client)
...
class TestUploadPhotos:
    ...
    @pytest.mark.asyncio
    async def test_upload_photos_file_naming(
        self,
        test_config: Config,
        mock_github_client: GitHubClient,
        mock_telegram_context,
    ...
    @pytest.mark.asyncio
    async def test_upload_photo_markdown_format(
        self,
        test_config: Config,
        mock_github_client: GitHubClient,
        mock_telegram_context,
...
class TestHandleMessageAuthorization:
    ...
    async def test_empty_whitelist_allows_all(
        self,
        test_config_no_whitelist: Config,
        mock_github_client: GitHubClient,
        create_mock_message,
        create_mock_update,
        mock_telegram_context,
...
class TestHandleMessageValidation:
    ...
    async def test_empty_message(
        ...
        message = create_mock_message(
            text=None, caption=None, photo=[], from_user=mock_telegram_user
        )
        update = create_mock_update(message=message, effective_user=mock_telegram_user)
...
class TestHandleMessageEndToEnd:
    ...
    async def test_text_only_message(
        ...
        message = create_mock_message(
            text="这是一段测试文字 #标签1 #标签2", from_user=mock_telegram_user
        )
        update = create_mock_update(message=message, effective_user=mock_telegram_user)
    ...
    @pytest.mark.asyncio
    async def test_single_photo_with_caption(
        self,
        test_config: Config,
        mock_github_client: GitHubClient,
        create_mock_message,
        ...
        message = create_mock_message(
            caption="图片说明 #图片", photo=photos, from_user=mock_telegram_user
        )
        update = create_mock_update(message=message, effective_user=mock_telegram_user)

        with (
            patch.object(handler.github, "upload_file") as mock_upload,
